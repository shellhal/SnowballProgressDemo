...
//TO-DO S:
// STANDARDIZE DISTANCE FUNCTIONS
// CLEAN UP FUNCTION ORDERING
//Get rid of debugging values (extra times 10s, extra divisions), things that arent physically based rendering
...
int dimensions = 8; //diameter of particle sphere
...
bool direct_collision = true; //whether you want snowballs to hit fly 
							   //directly at each other or your own choice of speeds
int count = 0;
float kElastic = .04;
float kDampening = 0;
float radius = 1.;
float kDist = 2*radius;
float mass = 1.;

bool inelastic = false;
float inelasticPercent = .5605;
...
void randomSlowDown(){
	...
	{
		if(particles[i].velocity[0] > t)
			particles[i].velocity[0] = t;

		if(particles[i].velocity[1] > t)
			particles[i].velocity[1] = t;

		if(particles[i].velocity[2] > t)
			particles[i].velocity[2] = t;
	}
	for (int i = 0; i < particlesB.size(); ++i)
	{
		if(particlesB[i].velocity[0] > t)
			particlesB[i].velocity[0] = t;

		if(particlesB[i].velocity[1] > t)
			particlesB[i].velocity[1] = t;

		if(particlesB[i].velocity[2] > t)
			particlesB[i].velocity[2] = t;
	}
}
/****************************************
...
* Description: init center and other misc info before creating particles
*****************************************/
void initScene(){
...
/****************************************
* Function Name: initColors
* Description: init colors after particles have been created
*****************************************/
void initColors(){
	for(int i = 0; i < particles.size(); i++){
...
float distance(particle a, particle b){
	...
	return( pow((pow(x,2) + pow(y,2) + pow(z,2)), 2));
}
...
void initConnections(){
	for(int i = 0; i < particles.size(); i++){
		//find all surroudnign connections
		//aka within their radius plus a tiny fraction
		//"connect" by adding to neighbors array
		//update size of neighbors array size
		for(int j = 0; j < particles.size(); j++){
			if(distance(particles[i], particles[j]) <= (radius*2.+.05)){
				if(particles[i].neighbors_size < 6 && i!=j) { //if we arent at max connection capacity, AND not same one
					particles[i].neighbors[particles[i].neighbors_size] = j; //add index of connection
					particles[i].neighbors_size++;
				}
			}
		}
	}
	...
	for(int i = 0; i < particlesB.size(); i++){
		for(int j = 0; j < particlesB.size(); j++){
			if(distance(particlesB[i], particlesB[j]) <= (radius*2.+.05)){
				if(particlesB[i].neighbors_size < 6 && i!=j) { //if we arent at max connection capacity, AND not same one
					particlesB[i].neighbors[particlesB[i].neighbors_size] = j; //add index of connection
					particlesB[i].neighbors_size++;
				}
			}
		}
	}
...
void initVelocities(){
	...
	if(direct_collision){	//head directly toward one another
		speed[0] = particlesBInfo.center[0] - particlesInfo.center[0] *10;	//THE TIMES 10 IS
		speed[1] = particlesBInfo.center[1] - particlesInfo.center[1]  *10; //JUST FOR DEBUGGING
		speed[2] = particlesBInfo.center[2] - particlesInfo.center[2]  *10;	//TRY AND FIX THINGS
	}
	else{
		speed[0] = -1;
		speed[1] = 1;
...
void updateSpringForce(){
	float distX, distY, distZ;//, distanceM;
	int a;
	for(int i= 0; i < particles.size(); i++){
		for (int j = 0; j < particles[i].neighbors_size; j++)
		{
			a = particles[i].neighbors[j]; //save the index
			distX = particles[a].x - particles[i].x ; //get vector in each direction
			distY = particles[a].y - particles[i].y ;
			distZ = particles[a].z - particles[i].z ;

			particles[i].velocity[0] += kElastic * distX *timeStep; //elastic times direction divided by unit vector
			particles[i].velocity[1] += kElastic * distY *timeStep;
			particles[i].velocity[2] += kElastic * distZ *timeStep;
		}
	}

	for(int i= 0; i < particlesB.size(); i++){
		for (int j = 0; j < particlesB[i].neighbors_size; j++)
		{
			a = particlesB[i].neighbors[j]; //save the index
			distX = particlesB[a].x - particlesB[i].x ; //get vector in each direction
			distY = particlesB[a].y - particlesB[i].y ;
			distZ = particlesB[a].z - particlesB[i].z ;

			particlesB[i].velocity[0] += kElastic * distX *timeStep; //elastic times direction divided by unit vector
			particlesB[i].velocity[1] += kElastic * distY *timeStep;
			particlesB[i].velocity[2] += kElastic * distZ *timeStep;
		}
	}
}
...
void updateAll(){
	...
	for(int i = 0; i < particles.size(); i++){
		...
		//Calculate gravity
		particles[i].velocity[1] += 1./1000.0*GRAVITY;
		//Update position
		particles[i].x += particles[i].velocity[0]*timeStep;
		particles[i].y += particles[i].velocity[1]*timeStep;
		particles[i].z += particles[i].velocity[2]*timeStep;


	}
	for(int i = 0; i < particlesB.size(); i++){
		particlesB[i].velocity[1] += 1./1000.0*GRAVITY;

		particlesB[i].x += particlesB[i].velocity[0]*timeStep;
		particlesB[i].y += particlesB[i].velocity[1]*timeStep;
		particlesB[i].z += particlesB[i].velocity[2]*timeStep;
	}
	randomSlowDown();
}
